...
var getColor = function (color, tokens, mode) {
    return tokens
        .filter(function (token) { return token.tags[0] === mode; })
        .map(function (token) {
        var _a = token.name.split('.'), category = _a[1], subcategory = _a[2], name = _a[3];
        var value = color[token.name];
        // TODO: Исправить этот метод, т.к. сейчас отдаются некорректные значения с прозрачностью
        var restoredValue = plasma_tokens_utils_1.getRestoredColorFromPalette(value);
        var tokenName = utils_1.kebabToCamel([subcategory === 'default' ? '' : subcategory, category, name].join('-'));
        return "/** " + token.description + " */\n        " + tokenName + ": '" + restoredValue + "',";
    })
        .join('\n        ');
};
var getContent = function (darkColor, lightColor) { return "export const color = {\n    dark: {\n        " + darkColor + "\n    },\n    light: {\n        " + lightColor + "\n    },\n};\n"; };
var createColorTokens = function (themeDir, color, tokens) { return __awaiter(void 0, void 0, void 0, function () {
    var darkColor, lightColor, content;
    return __generator(this, function (_a) {
        if (!(tokens === null || tokens === void 0 ? void 0 : tokens.length)) {
            return [2 /*return*/];
        }
        darkColor = getColor(color, tokens, 'dark');
        lightColor = getColor(color, tokens, 'light');
        content = getContent(darkColor, lightColor);
        utils_1.writeTokens(content, themeDir, 'color.ts');
        return [2 /*return*/];
    });
}); };
